...
basic_route = fr"C:\Users\1012986131\Desktop\省测统计数据- 10.15\学校数据"
...
for item in find_files_by_extension(folder_path=basic_route, extension=".xlsx"):
    ...
    for item in data[1:]:
        temp = [subject] + item
        output[item[1]].append(temp)


for key,value in output.items():
    for subject in subject_list:
        save_excel(two_dimension_list=[title] + [item for item in value if item[0] == subject],excel_name=f"指导中心下发数据/{key}/{subject}学校数据")
