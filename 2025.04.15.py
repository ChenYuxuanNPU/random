...
# 读取JSON文件
with open('answer_student.json', 'r', encoding='utf-8') as file:
    json_data = dict(json.load(file))

raw_data = read_xlsx_to_list(file_path="2.xlsx")
...
for school_name in output_dict.keys():
    ...
    with open(f"output_student/{school_name}.txt", mode="w", encoding="UTF-8") as f:
        print(f"{school_name}学生问卷数据", file=f)
...
sum_stat = {f"问题{i}": {f"{chara}": 0 for chara in ["A", "B", "C", "D"]} for i in range(1, 14)}
# todo
...
save_excel(two_dimension_list=output, excel_name="output_student/合计表格")

temp ={s:{} for s in ["广州市白云区同和中学", "广州市白云区东平学校"]}
for school in ["广州市白云区同和中学", "广州市白云区东平学校"]:
    count = 1
    with open(rf"0416/{school}-学生.txt", "w", encoding='UTF-8') as f:
            ...
            if len(list(data[qes].keys())) <= 5:
                ...
                for ans in data[qes].keys():
                    if ans != "总数" and data[qes][ans] > 0:
                        print(
                            f"{data[qes][ans]}位学生认为“{ans}”，占比{str(round(data[qes][ans] / data[qes]['总数'] * 100, 4))[:5]}%",
                            file=f, end=";")
